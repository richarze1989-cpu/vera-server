...
const CHATWOOT_INBOX_ID = process.env.CHATWOOT_INBOX_ID; // 113329
...
// ─── CHATWOOT — RESPONDER EN CONVERSACIÓN ────────────────────────────────────
...
// ─── CHATWOOT WEBHOOK — Recibe mensajes desde Chatwoot ───────────────────────
...
app.post('/chatwoot-webhook', async (req, res) => {
  try {
    ...
    // Usar conversationId como clave del historial
    ...
    if (from) {
      if (detectarIntencionDeposito(text)) {
        await enviarAlerta(from, obtenerResumen(conversaciones[key]));
      }
      if (detectarConsultaDisponibilidad(text)) {
        await enviarAlertaDisponibilidad(from, obtenerResumen(conversaciones[key]));
      }
    }
    ...
    // Enviar respuesta via Chatwoot (Chatwoot la envía a WhatsApp automáticamente)
...
// ─── AUTO-PING (mantener servidor activo en Render plan gratuito) ─────────────
